...
#  scripts/build-aartool.sh, the same way aartool-baseline.sh is built from
#  scripts/src/. Edit the sources and rebuild.
#
#  WHY THIS EXISTS
#  The toolkit had two entry points with two incompatible conventions for one
#  workflow. aartool-baseline.sh took --host and --user; run-hardening.sh took
...
#  independently usable, and aartool-baseline.sh keeps the single-file
